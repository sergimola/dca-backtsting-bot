...
- **FR-020**: Upon receiving a valid promotion request, the API MUST spawn a Go engine batch execution with `wide_events_to_stdout: true` for ONLY the submitted configurations, using the server-retrieved `config_json` (not client-supplied data). The `wide_events_to_stdout` field is a new boolean config key (default `false`) introduced by this spec that routes wide events directly to the engine's stdout stream rather than to a temporary `.jsonl` file (see research D-006).
- **FR-021**: The Go engine MUST stream the resulting wide events to stdout (enabled by `wide_events_to_stdout: true`), with each event JSON line including the `run_id` field (per FR-005b). The Node.js API layer MUST intercept these wide events from the engine's stdout stream, read the `run_id` from each event to attribute it correctly, and bulk-insert them into the ClickHouse `sweep_wide_events` table tagged with the parent `session_id`. The Go engine MUST NOT write directly to ClickHouse for wide events.
...
- **FR-025**: Before inserting wide events for a batch of promoted runs, the system MUST execute a single `ALTER TABLE sweep_wide_events DELETE WHERE session_id = '<session_id>' AND run_id IN ('<run_id1>', '<run_id2>', ...)` mutation covering all previously-promoted `run_id`s in the batch. This MUST be a single mutation regardless of how many `run_id`s are in the batch — a per-run-loop of individual DELETE mutations is explicitly forbidden (see research D-003). This pre-deletion strategy provides immediate consistency without requiring `FINAL` on Grafana read queries.
...
- The Go engine already supports `enable_wide_events` as a config payload boolean (spec 017 FR-025/FR-026) for writing wide events to a temporary `.jsonl` file. This spec introduces a separate `wide_events_to_stdout` boolean config field for the batch promotion path, which routes wide events to stdout instead of a file, enabling the Node.js API relay pattern (see research D-006).
